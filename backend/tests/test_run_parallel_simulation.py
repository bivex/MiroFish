...
def _load_module():
    module_name = 'scripts.run_parallel_simulation'
    path = Path(__file__).resolve().parents[1] / 'scripts/run_parallel_simulation.py'

    dotenv_mod = types.ModuleType('dotenv')
    dotenv_mod.load_dotenv = lambda *args, **kwargs: None
    sys.modules['dotenv'] = dotenv_mod

    action_logger_mod = types.ModuleType('action_logger')
    action_logger_mod.SimulationLogManager = object
    action_logger_mod.PlatformActionLogger = object
    sys.modules['action_logger'] = action_logger_mod
...
def test_sanitize_agent_tool_schemas_removes_empty_required_for_zero_arg_tool():
    ...
    patched = mod._sanitize_agent_tool_schemas(_FakeGraph(tool))
...
def test_sanitize_tool_schema_ignores_tools_with_real_parameters():
    ...
    assert mod._sanitize_tool_schema(tool) is False
