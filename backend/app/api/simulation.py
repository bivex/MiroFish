...
def create_simulation():
    ...
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "Project graph has not been built yet. Call /api/graph/build first."
            }), 400
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        if status in prepared_statuses and config_generated:
            ...
            if enable_reddit and os.path.exists(reddit_profiles_file):
                with open(reddit_profiles_file, 'r', encoding='utf-8') as f:
                    profiles_data = json.load(f)
                    profiles_count = len(profiles_data) if isinstance(profiles_data, list) else 0
            elif enable_twitter and os.path.exists(twitter_profiles_file):
    ...
    返回：
        {
            ...
            "data": {
                ...
                "message": "Preparation task started|Existing preparation already available",
            ...
            if is_prepared:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "message": "Existing preparation already available; no regeneration needed",
            ...
            if is_prepared:
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "progress": 100,
                        "message": "Existing preparation already available",
                        "already_prepared": True,
                        "prepare_info": prepare_info
                    }
                })
    ...
    try:
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                ...
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": "Simulation is already running. Call /stop first or use force=true to restart."
                            }), 400
                ...
                return jsonify({
                    "success": False,
                    "error": f"Simulation is not ready. Current status: {state.status.value}. Call /prepare first."
                }), 400
        ...
        if enable_graph_memory_update:
            ...
            if not graph_id:
                return jsonify({
                    "success": False,
                    "error": "A valid graph_id is required to enable graph memory updates. Please build the graph first."
                }), 400
...
def interview_agent():
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment is not running or has been closed. Make sure the simulation finished and is waiting for commands."
            }), 400
    ...
    try:
        ...
        for i, interview in enumerate(interviews):
            ...
            if item_platform and item_platform not in ("twitter", "reddit"):
                return jsonify({
                    "success": False,
                    "error": f"Interview item {i+1} has an invalid platform. Allowed values: 'twitter' or 'reddit'"
                }), 400
        ...
        return jsonify({
            "success": False,
            "error": f"Timed out while waiting for batch interview responses: {str(e)}"
        }), 504
