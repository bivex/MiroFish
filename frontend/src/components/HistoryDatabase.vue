        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step 3 “Run Simulation” and Step 5 “Deep Interaction” require a live session and cannot be replayed from history.</span>
            </div>
          </div>
        </div>
      </Transition>
...
const loadHistory = async () => {
  try {
    ...
    if (response.success) {
      projects.value = sortProjectsByRecent(response.data || [])
    }
  } catch (error) {
